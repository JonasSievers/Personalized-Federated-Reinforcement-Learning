...
from torchrl.envs.transforms import TransformedEnv, InitTracker, Compose, ObservationNorm
import torch
...
def createEnvSpecs(env_params):
    ...
    # observation_spec = Composite(observation=Unbounded(shape=(1 * env_params.forecast_horizon + 2,), dtype=torch.float32))
...
def createDatasets(cfg):
    return [EnergyDataset(data_path=cfg.experiment.dataset_path,mode=mode) for mode in ['test', 'eval', 'train']]
